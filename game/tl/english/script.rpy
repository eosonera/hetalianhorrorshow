# TODO: Translation updated at 2025-08-05 20:52

# game/script.rpy:58
translate english start_4a1398f6:

    # na "「７カ国が集まる国際怖い話会合があるからそこで話を聞いてきてくれないかなー？ \n\n{space=200}その一言でそれは始まった。"
    na "\"Seven nations will gather at the International Meeting of Scary Stories, do you want to come and listen to their stories?\"\n\n{space=200}It started with that one line."

# game/script.rpy:68
translate english start_d72426f3:

    # bul "えっ俺が？"
    bul "Wha— me?"

# game/script.rpy:72
translate english start_4d123f7b:

    # rom "ごめんね~。\nほんとはおいらが\nやりたかったんだけどさー。"
    rom "Sorry～\nI really wanted to go and do it\nmyself, but, y’know..."


# game/script.rpy:82
translate english start_6074a98b:

    # bul "いやそれ俺もだけどね。\n俺も遊ぶ暇あったら\n内職しろって\n言われてるんだわー。"
    bul "Nah, I get it, same here. He tells me if I have time to play around, I should go get a side hustle."

# game/script.rpy:86
translate english start_8f7e5e1b:

    # rom "そうだったの！？\nでもお前いっつも\nフラフラしてるじゃんかー！"
    rom "He says that!? But you're allllways just messin' around, aren't you?!"

# game/script.rpy:90
translate english start_fe5ba20e:

    # bul "ふざけんなー！\n自分で行けオラァ！"
    bul "Screw you! You're going yourself!"

# game/script.rpy:94
translate english start_d6cfb26b:

    # rom "なんて友だち甲斐が\nないんだよー！頼むよー！\n頼める人がいないんだよー"
    rom "That's not something a friend would sayyy! I'm counting on youuu-! There's no one else I can count ooon."

# game/script.rpy:98
translate english start_c66e6f28:

    # bul "ロシアさんは？"
    bul "What about Mr. Russia?"

# game/script.rpy:106
translate english start_4a9432bb:

    # bul "そこら辺は\n…否定はしないわー"
    bul "I can't...\nreally deny that."

# game/script.rpy:110
translate english start_c71f0dd7:

    # rom "お願いだよー！！\nお菓子あげるから！"
    rom "Pleeease!!?\nI'll give you candy!"

# game/script.rpy:118
translate english start_b3b001cd:

    # rom "ブルガリア今年何歳？"
    rom "How old are you this year?"

# game/script.rpy:122
translate english start_1fcc0816:

    # bul "んー ま、\nかれこれ…３？"
    bul "Hmm... Well, somewhere around...\n3...?"

# game/script.rpy:126
translate english start_3d397b61:

    # rom "３００じゃないよね?"
    rom "You're not 300 years old,\nare you?"

# game/script.rpy:130
translate english start_59e783b8:

    # bul "んーん♪"
    bul "Nn~ope♪"

# game/script.rpy:134
translate english start_d37301ef:

    # rom "３０００？"
    rom "3000?"

# game/script.rpy:138
translate english start_239fc5ac:

    # bul "もう一声ぇっ！"
    bul "Try again!"

# game/script.rpy:142
translate english start_6b148d73:

    # rom "３万！？ ないよー！"
    rom "30,000!? There's no way!"

# game/script.rpy:148
translate english start_97b31921:

    # bul "俺三万歳！！\nつまり俺の国力は\n３万あるんだわー！"
    bul "I am 30,000! Which means my national power is 30,000 too!"

# game/script.rpy:156
translate english start_e5a62779:

    # rom "こいつ盛ったぁー！！"
    rom "You're really reaching!!"

# game/script.rpy:163
translate english start_300040ae:

    # bul "盛ってねーから！\nそういうわけで俺の方が\n年上だから俺はいかない！"
    bul "Am not! So yeah, since I'm your senior, I'm not going!"

# game/script.rpy:167
translate english start_c62956d7:

    # rom "やーだー！その理屈わかんないよー\n頼むよー！行ってよー！\nいらとお前の付き合いだろー！！"
    rom "Nooo! You don't understand! Pleeease! Just go! This is why we have each other!!"

# game/script.rpy:173
translate english start_091ebbd4:

    # bul "ぷぷぷぷ～\nぷんにょにょ～（鼻歌）"
    bul "Hm-hm-hm-hm~\nHm-mm-mm~ (humming)"

# game/script.rpy:178
translate english start_5ef33473:

    # rom "やだー変な歌うたいながら\nおいらの周りまわらないでー。\nやめてよー"
    rom "Nooo, don't walk around me\nhumming a weird song! Stooop!"

# game/script.rpy:181
translate english start_76f6ac0e:

    # bul "ふんごっ\nふんごっ（鼻歌）"
    bul "Hmm-mmh\nhmm-mmh (humming)"

# game/script.rpy:185
translate english start_fcb03828:

    # rom "わーんわーん！！"
    rom "Waaah! Waaah!"

# game/script.rpy:187
translate english start_7c634a5e:

    # rom "どうすればいいんだよぅこれー。"
    rom "What should I do nooow?"

# game/script.rpy:191
translate english start_aa940026:

    # rom "\n…あっ！そうだ！\nこの仕事すると…目立てるよ！"
    extend "\n...Ah! I've got it! If you do go...\nYou'll stand out!"

# game/script.rpy:196
translate english start_63665121:

    # bul "何！！？"
    bul "What!!?"

# game/script.rpy:200
translate english start_028c8b01:

    # rom "他のみんなと対等な目線で\n話せるよー！"
    rom "You'll be able to speak on the same level as everyone else!"

# game/script.rpy:204
translate english start_c0e5473b:

    # bul "つまり…\nどういうことだ？"
    bul "So...\nLike what?"

# game/script.rpy:208
translate english start_ebbafdad:

    # rom "だから！司会進行として！\nいつもは上からにゃーにゃー\n言ってくるみんなをブルガリアが\n先導できるってこと！！"
    rom "You see! As the moderator! You'll be able to lead all the people who used to just meow at us from above!!"

# game/script.rpy:212
translate english start_15292df2:

    # rom "思い出してよ\nおいらたちの近代…"
    rom "You remember our recent history..."

# game/script.rpy:217
translate english start_d9ee66bd:

    # bul "俺たちの近代…"
    bul "Our recent history..."

# game/script.rpy:224
translate english start_70786b2a:

    # na2 "またブルガリアと\nルーマニアか…。"
    na2 "Bulgaria and Romania again...?"

# game/script.rpy:226
translate english start_ab6fbfcb:

    # na2 "この二国って加入してから\n足引っ張ってるだけですよね。\nだから反対してたんですよ。"
    na2 "Those two countries have just been dragging their feet since they joined. That's why I opposed it."

# game/script.rpy:228
translate english start_b1038b62:

    # na2 "せんせー！\nブルガリア君とルーマニア君が\n出稼ぎに来て邪魔臭いです。"
    na2 "Mister! Bulgaria and Romania came over for work, and it's a big pain."

# game/script.rpy:230
translate english start_cc76127d:

    # na2 "ブルガリア君とルーマニア君って\nＥＵに必要ですか？\nこいつら追い出してトルコ入れた方が\n有意義だと思いますー"
    na2 "Does the EU really need Bulgaria and Romania? I think it would be better to kick them out and accept Turkey."

# game/script.rpy:239
translate english start_1db23734:

    # bul "うう…、ちくしょうちくしょう…。\nユーロ圏の新聞めー…。\n人の気持ちもしらねーで\n普通にこういう事書くんだわ…。\n手厳しいんだわー…。うっうっ…。\n金がねェンだよ…！国内建て直す\n金が手に入るまで働くしかねぇんだわ…。"
    bul "Ugh... dammit, dammit!\nDamn Eurozone newspapers...\nThey're just writin' this stuff without\nknowin' how the people really feel...\nIt's so harsh... *sob*...\nI dun have any money...!\nOf course I hafta work until I have\nenough money to rebuild my country..."

# game/script.rpy:243
translate english start_1bce4555:

    # rom "泣かないでよぉブルガリア…！\nおいらブルガリアを悲しませたくて\n思い出させたわけじゃないよぉ…"
    rom "Don't cry, Bulgariaa! I didn't remind you because I wanted to make you saaad..."

# game/script.rpy:247
translate english start_1530bf19:

    # bul "俺、やる…！"
    bul "I'll, I'll do it...!"

# game/script.rpy:251
translate english start_d4d62c12:

    # rom "ブルガリア…！！"
    rom "Bulgaria...!!"

# game/script.rpy:256
translate english start_fc180838:

    # bul "ちょっくら司会進行として\n国としての存在感\nアピッてくるんだわ！！"
    bul "They'll see, as the moderator, I'll definitely show off my presence as a country!!"

# game/script.rpy:260
translate english start_0f5baf6e:

    # rom "そうだよブルガリア！\nその意気だよ\n頑張ろうねぇ～！！"
    rom "That's right, Bulgaria!\nThat's the spirit!\nDo your best~!!"

# game/script.rpy:267
translate english start_c6e01564:

    # bul "そしてこのゲームの\n主役も頂くんだわ！"
    bul "And I'll be this game's\nmain character!"

# game/script.rpy:271
translate english start_caf63c9d:

    # rom "…え！？"
    rom "...Huh!?"



# TODO: Translation updated at 2025-08-07 18:52

# game/script.rpy:70
translate english start_3735337e:

    # rom "{size=-2}魔術部のイギ…\nほにゃららさんに{size=-5}ごほごほっ{/size}\nいつやるんだって\nせっつかれてるしー…。{/size}"
    rom "{size=-2}One of the Magic Club members, Eng... \nEr, Mr. Blahblah {size=-5}(cough cough),{/size}\nkeeps pestering me about\nwhen I'm going to go...{/size}"

# game/script.rpy:73
translate english start_fe490979:

    # rom "{size=-2}でも…その、上司に、お前\nそんな事してる暇ないだろって\n言われちゃってー…。{/size}"
    rom "{size=-2}But... well, my boss says I don't have the time for such things...{/size}"

# game/script.rpy:101
translate english start_d7ed91dd:

    # rom "え！？ えーと…。\nロシアさんは{size=-5}存在自体がー…\n怖いっていうか…。{/size}"
    rom "Wha!? Um...\nWell, Mr. Russia's {size=-5}mere existence...\nIs pretty scary...{/size}"

# game/script.rpy:296
translate english start_8302b42c:

    # na "こうしてブルガリアさんは、\n集められた７カ国から怖い話を聞く\nべく今日の会場へと向かう事になったのだった。"
    na "And that's how Mr. Bulgaria ends up heading for the meeting's venue to hear the scary stories of the seven countries gathered there today."

# TODO: Translation updated at 2025-08-09 15:30

# game/script.rpy:14
translate english start_051b4992:

    # na "「７カ国が集まる国際怖い話会合があるから \n　そこで話を聞いてきてくれないかなー？」\n{vspace=12} {space=155}その一言でそれは始まった。"
    na "{size=-2}\"Seven nations will gather at the International Meeting of Scary Stories, do you want to come and listen to their stories?\"\n{vspace=12} {space=155}It started with that one line.{/size}"

# game/script.rpy:78
translate english start_bc7df12d:

    # bul "お菓子でつられる\n年じゃねーんだわー"
    bul "I'm too old to be bribed with candy."

